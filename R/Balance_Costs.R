#' @title Balance Holding and Ordering Costs Heuristic for lot-sizing with time-varying demand
#'
...
#' 
#' The Balance Holding and Ordering Costs algorithm is a heuristic alternative to the optimizing dynamic programming
...
#' See sections 4.5, 4.6, and 4.8 in Axsäter's book. 
#'   
#' @references
...
#'
#' @seealso   Wagner_Whitin, Silver_Meal
#' 
...
#' ##
#' 
#' Balance_Costs(d=c(50, 60, 90, 70, 30, 100, 60, 40, 80, 20), A=300, h=1)
#' 
...
#' ##
#' 
#' Balance_Costs(d=c(90, 120, 80, 70), A=500, h=2)
#' 
#' ## ** Example 3 **
...
#' ##
#' 
#' Balance_Costs(d=c(90, 120, 80, 70), A=500, v=20, r=0.1)
#' 
#' 
#' @export
Balance_Costs <- function(d,A,h=0,v=0, r=0) {
  L  <- length(d) 
  ...
  repeat{
    ...
    accumulated_hcost <- accumulated_hcost + holding_cost
    if ( accumulated_hcost <= A ){
  ...
  total_cost <- total_cost + total_order_cost + total_holding_cost
